...
def is_validated_english_sentence(user_input):
    ...
    # ===Modify codes below=============
    # 조건에 따라 변환되어야 할 결과를 result 변수에 할당 또는 필요에 따라 자유로운 수정
    not_variable = re.compile("[^a-zA-Z.,?! ]+")
    if len(''.join(re.findall(not_variable,user_input))) != 0:
        return False
    if len(''.join(re.findall("[a-zA-Z]+",user_input))) == 0:
        return False
    return True
    # ==================================
...
def is_validated_morse_code(user_input):
    ...
    # ===Modify codes below=============
    # 조건에 따라 변환되어야 할 결과를 result 변수에 할당 또는 필요에 따라 자유로운 수정
    if len(''.join(re.findall("[^-. ]+",user_input))) != 0:
        return False        
    for code in user_input.split():
        if not(code in list(get_morse_code_dict().values())):
            return False
    return True
    # ==================================
...
def decoding_character(morse_character):
    ...
    # ===Modify codes below=============
    # 조건에 따라 변환되어야 할 결과를 result 변수에 할당 또는 필요에 따라 자유로운 수정
    morse_code_reverse = { v : k for k,v in get_morse_code_dict().items() }

    return morse_code_reverse.get(morse_character, ' ')
    # ==================================
